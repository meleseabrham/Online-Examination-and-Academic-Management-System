...
const ManageTeacherAssignments = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [assRes, teachRes, classRes, courseRes, ayRes, semRes] = await Promise.all([
                axios.get('http://localhost:5000/api/admin/assignments/teachers', { headers }),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingAssignmentId) {
                await axios.put(`http://localhost:5000/api/admin/assignments/teachers/${editingAssignmentId}`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                await axios.post('http://localhost:5000/api/admin/assignments/teachers', formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            await fetchData();
    ...
    const handleDelete = async (id: number) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://localhost:5000/api/admin/assignments/teachers/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchData();
        } catch (err) {
                ...
                <div id="scrollable-body" className="flex-1 overflow-y-auto p-4 md:p-8 pt-2 scroll-smooth">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                        ...
                        <button
                            ...
                            className="bg-brand-blue text-white px-8 py-4 rounded-[22px] font-black shadow-xl shadow-blue-500/20 hover:bg-blue-600 transition-all flex items-center gap-3 active:scale-95"
                        >
                            <UserPlus size={20} />
                            New Assignment
                        </button>
