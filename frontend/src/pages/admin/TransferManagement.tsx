...
const TransferManagement = () => {
    ...
    const fetchSchools = async () => {
        try {
            const res = await axios.get('http://localhost:5000/api/admin/schools', { headers });
            setSchools(res.data);
    ...
    const fetchSections = async (gradeId: string) => {
        try {
            const res = await axios.get(`http://localhost:5000/api/admin/sections?gradeId=${gradeId}`, { headers });
    ...
    const fetchTransferHistory = async () => {
        setLoading(true);
        try {
            const res = await axios.get('http://localhost:5000/api/admin/transfers/history', { headers });
    ...
    const handleSchoolSubmit = async () => {
        try {
            if (editingSchool) {
                await axios.put(`http://localhost:5000/api/admin/schools/${editingSchool.Id}`, schoolForm, { headers });
                showMsg('success', 'School updated successfully');
            } else {
                await axios.post('http://localhost:5000/api/admin/schools', schoolForm, { headers });
    ...
    const handleDeleteSchool = async (id: number) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/api/admin/schools/${id}`, { headers });
    ...
    const handleStudentTransfer = async () => {
        ...
        setLoading(true);
        try {
            await axios.post('http://localhost:5000/api/admin/transfers/student', {
    ...
    const handleTeacherTransfer = async () => {
        ...
        setLoading(true);
        try {
            await axios.post('http://localhost:5000/api/admin/transfers/teacher', {
