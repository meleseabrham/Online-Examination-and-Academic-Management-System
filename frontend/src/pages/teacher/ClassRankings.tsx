...
const ClassRankings = () => {
    ...
    const fetchInitialData = async () => {
        try {
            const [yearsRes, gradesRes] = await Promise.all([
                axios.get(`http://localhost:5000/api/${isAdmin ? 'admin' : 'teacher'}/academic-years`, { headers }),
                axios.get(`http://localhost:5000/api/${isAdmin ? 'admin' : 'teacher'}/grades`, { headers })
            ]);
            ...
            if (activeYear) {
                ...
                const semRes = await axios.get(`http://localhost:5000/api/${isAdmin ? 'admin' : 'teacher'}/semesters?academicYearId=${activeYear.Id}`, { headers });
    ...
    const fetchSemesters = async (ayId: number) => {
        try {
            const res = await axios.get(`http://localhost:5000/api/${isAdmin ? 'admin' : 'teacher'}/semesters?academicYearId=${ayId}`, { headers });
    ...
    const fetchSections = async (gradeId: number) => {
        try {
            const url = isAdmin
                ? `http://localhost:5000/api/admin/sections?gradeId=${gradeId}${selectedAY ? `&academicYearId=${selectedAY}` : ''}`
    ...
    const fetchRankings = async () => {
        ...
        try {
            ...
            const endpoint = isSemester
                ? (isAdmin ? '/api/admin/semester-rankings' : '/api/teacher/semester-rankings')
                : (isAdmin ? '/api/admin/rankings' : '/api/teacher/rankings');
    ...
    const handleTriggerCalculation = async () => {
        ...
        setCalculating(true);
        try {
            await axios.post('http://localhost:5000/api/admin/calculate-final-rankings', { academicYearId: selectedAY }, { headers });
            fetchRankings();
        } catch (err) {
