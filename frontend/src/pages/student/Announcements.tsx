        ...
        <div className="flex bg-[#F4F7FE] h-screen overflow-hidden font-display">
                ...
                <div id="scrollable-body" className="flex-1 overflow-y-auto p-4 md:p-8 pt-2 scroll-smooth">
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[40px] shadow-sm border border-slate-100">
                            ...
                            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Syncing school updates...</p>
                        </div>
                        ...
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {paginatedAnnouncements.map((item) => (
                                    <div
                                        key={item.Id}
                                        className="bg-white p-8 rounded-[40px] shadow-sm border-2 border-transparent hover:border-brand-blue/10 hover:shadow-xl transition-all group relative overflow-hidden flex flex-col"
                                    >
                                        ...
                                        <button
                                            ...
                                            className="flex items-center gap-2 text-brand-blue font-black text-xs uppercase tracking-widest group/btn mt-auto"
                                        >
                                            Read Full Message
                            ...
                            {/* Pagination */}
                            {announcements.length > 0 && (
                                <div className="flex flex-col sm:flex-row justify-between items-center mt-10 gap-2 pb-8">
            ...
            {selectedAnnouncement && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[50px] max-w-2xl w-full p-10 relative shadow-2xl animate-in zoom-in-95 duration-300">
                        <button
                            onClick={() => setSelectedAnnouncement(null)}
                            className="absolute top-8 right-8 w-12 h-12 rounded-full bg-slate-50 text-slate-400 hover:text-[#2B3674] hover:bg-slate-100 flex items-center justify-center transition-all"
                        >
                            <X size={24} />
                        </button>
                        ...
                        <div className="prose prose-slate max-w-none">
                            <p className="text-[#2B3674]/80 text-lg font-medium leading-relaxed whitespace-pre-wrap max-h-[40vh] overflow-y-auto pr-4 custom-scrollbar">
                        ...
                        <button
                            onClick={() => setSelectedAnnouncement(null)}
                            className="mt-10 w-full bg-[#111C44] text-white py-5 rounded-[22px] font-black text-sm uppercase tracking-widest hover:bg-brand-blue transition-all shadow-xl shadow-[#111C44]/20 active:scale-[0.98]"
                        >
                            Close Announcement
                        </button>
                    </div>
