...
const AdminAssessmentResults = () => {
    ...
    useEffect(() => {
        const fetchSections = async () => {
            ...
            try {
                const res = await axios.get(`http://localhost:5000/api/director/sections?gradeId=${selectedGrade}&academicYearId=${selectedYear}`, { headers });
    ...
    const fetchStudents = async () => {
        ...
        setLoading(true);
        try {
            let url = `http://localhost:5000/api/director/enrollments?academicYearId=${selectedYear}&gradeId=${selectedGrade}`;
    ...
    const viewStudentMarks = async (student: Student) => {
        ...
        setLoadingBreakdown(true);
        try {
            const res = await axios.get(`http://localhost:5000/api/director/student-course-breakdown?studentId=${student.StudentId}&semesterId=${selectedSemester}`, { headers });
            setCourseBreakdown(res.data);
        } catch (err) {
            console.error(err);
        } finally {
