...
const ManageCourses = () => {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:5000/api/director/courses', {
    ...
    const fetchTeachers = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('http://localhost:5000/api/director/users', {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchSections = async (gradeId: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('http://localhost:5000/api/director/sections', {
                headers: { Authorization: `Bearer ${token}` },
                params: { gradeId }
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingCourse) {
                await axios.put(`http://localhost:5000/api/director/courses/${editingCourse.CourseId}`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                await axios.post('http://localhost:5000/api/director/courses', formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            fetchCourses();
    ...
    const handleDelete = async (id: number) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://localhost:5000/api/director/courses/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchCourses();
        } catch (err) {
    ...
    const handleUpdateAssignment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:5000/api/director/assignments/teachers/${editingAssignment.AssignmentId}`, {
