...
const ViewResults = () => {
    ...
    const isAdmin = (user.role?.toLowerCase() === 'admin' || user.role?.toLowerCase() === 'director');
    ...
    useEffect(() => {
        const fetchSections = async () => {
            ...
            try {
                ...
                const url = isAdmin
                    ? `http://localhost:5000/api/director/sections?gradeId=${selectedGrade}${selectedYear ? `&academicYearId=${selectedYear}` : ''}`
    ...
    const handleRowClick = (attemptId: number) => {
        const prefix = isAdmin ? '/director' : '/teacher';
