...
const ManageClasses = () => {
    ...
    const fetchAvailableStudents = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('http://localhost:5000/api/director/students/unassigned', {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchGrades = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('http://localhost:5000/api/director/grades', {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleClassSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingClass) {
                await axios.put(`http://localhost:5000/api/director/classes/${editingClass.ClassId}`, classForm, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                ...
                await axios.post('http://localhost:5000/api/director/classes', classForm, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            fetchClasses();
    ...
    const handleDeleteClass = async (id: number) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://localhost:5000/api/director/classes/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchClasses();
        } catch (err) {
    ...
    const handleEnterStudentManagement = async (cls: ClassData) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await axios.get(`http://localhost:5000/api/director/classes/${cls.ClassId}/students`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleAddStudent = async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.post('http://localhost:5000/api/director/classes/assign', {
            ...
            const response = await axios.get(`http://localhost:5000/api/director/classes/${selectedClass.ClassId}/students`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setClassStudents(response.data);
    ...
    const handleRemoveStudent = async (studentId: number) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.post('http://localhost:5000/api/director/classes/remove-student', {
                studentId,
                classId: selectedClass.ClassId
            }, {
                headers: { Authorization: `Bearer ${token}` }
    ...
    const handleStudentEditSubmit = async (e: React.FormEvent) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.put(`http://localhost:5000/api/director/users/${editingStudent.UserId}`, {
