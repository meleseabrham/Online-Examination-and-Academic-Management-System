 ...
 */
export const performFullBackup = async (req?: Request, res?: Response) => {
    try {
        ...
        if (res) res.json({ message: 'Full backup completed successfully', fileName });
        return { success: true, fileName };
    } catch (err: any) {
        console.error('Full Backup Error:', err);
        if (res) res.status(500).json({ message: 'Full backup failed', error: err.message });
...
export const restoreDatabase = async (req: Request, res: Response) => {
    ...
    if (!fileName) return res.status(400).json({ message: 'Backup file name is required' });
