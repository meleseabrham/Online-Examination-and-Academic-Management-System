...
export const generateTranscript = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            settingsRes.recordset.forEach((s) => globalSettings[s.SettingKey] = s.SettingValue);
        }
        catch (e) {
            console.log("SystemSettings table not ready yet");
        }
...
// =============================================
export const generateFullYearTranscript = async (req, res) => {
    ...
    try {
        ...
        const processedBySemester = {};
        for (const sId of Object.keys(dataBySemester)) {
            processedBySemester[Number(sId)] = Object.values(dataBySemester[Number(sId)]).map((c) => ({
                ...c,
        ...
        doc.fontSize(9).font('Helvetica-Bold').fillColor('#333');
        doc.text('Student:', 50, infoY);
        doc.font('Helvetica').text(student.FullName, 100, infoY);
        doc.font('Helvetica-Bold').text('Reg #:', 230, infoY);
        doc.font('Helvetica').text(student.RegistrationNumber || 'N/A', 270, infoY);
        doc.font('Helvetica-Bold').text('Gender:', 380, infoY);
        doc.font('Helvetica').text(student.StudentGender || 'N/A', 420, infoY);
        doc.font('Helvetica-Bold').text('Grade:', 50, infoY + 15);
        doc.font('Helvetica').text(`Grade ${student.GradeNumber} - ${student.SectionName}`, 100, infoY + 15);
        doc.y = infoY + 65;
        doc.moveTo(50, doc.y).lineTo(50 + pageWidth, doc.y).stroke('#ddd');
        doc.moveDown(1.5);
        // --- DATA PREPARATION ---
        const yearlyCourseData = {};
        ...
        for (const semester of semesters) {
            ...
            courses.forEach((course) => {
                ...
                if (!yearlyCourseData[course.name])
                    yearlyCourseData[course.name] = {};
        ...
        if (cCourseNames.length > 0) {
            ...
            semesters.forEach((sem, i) => {
                const sX = colX[2 + i];
                if (i > 0)
                    doc.moveTo(sX, currentY + rowHeight).lineTo(sX, currentY + rowHeight * 2).stroke();
