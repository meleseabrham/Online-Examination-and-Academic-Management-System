...
export const ensureSchema = async (pool) => {
    ...
    try {
        ...
        console.warn('[Schema] Migration warning (non-fatal):', err.message);
    ...
    try {
        ...
        if (semesterId) {
            ...
            if (semRes.recordset.length > 0)
                ayId = semRes.recordset[0].AcademicYearId;
        }
        // Start Transaction
...
export const getAvailableMarks = async (req, res) => {
    const { courseId, classId } = req.query;
    if (!courseId || !classId)
        return res.status(400).json({ message: 'Missing courseId or classId' });
    try {
        const pool = await poolPromise;
        await ensureSchema(pool);
 ...
 */
export const updateExpiredAssignments = async (pool, studentId) => {
    try {
        const request = pool.request();
        let query = `
            UPDATE sea
 ...
 */
export const markMissedExams = async (req, res) => {
 ...
 */
export const getMissedStudents = async (req, res) => {
    const { examId } = req.params;
    try {
        const pool = await poolPromise;
        await ensureSchema(pool);
 ...
 */
export const reassignExamToStudents = async (req, res) => {
    const teacherId = req.user.id;
 ...
 */
export const createMakeupFromMissed = async (req, res) => {
    const teacherId = req.user.id;
