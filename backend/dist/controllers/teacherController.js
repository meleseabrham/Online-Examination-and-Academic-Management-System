...
export const getMyRecentExams = async (req, res) => {
    const teacherId = req.user.id;
    try {
        const pool = await poolPromise;
...
export const getLiveExamSessions = async (req, res) => {
    ...
    try {
        ...
        let filterSql = '';
        if (ayId)
            filterSql += ' AND e.AcademicYearId = @ayId';
        if (semesterId)
            filterSql += ' AND e.SemesterId = @semId';
        if (courseId)
            filterSql += ' AND e.CourseId = @courseId';
        if (examType)
            filterSql += ' AND e.ExamType = @examType';
        if (gradeId) {
        ...
        res.json({
            ...
            totalActive: liveStudents.recordset.filter((s) => s.Status === 'Started').length
        });
    }
    catch (err) {
...
export const gradeStudentExam = async (req, res) => {
    const { attemptId } = req.params;
...
export const unlockAttempt = async (req, res) => {
    const { attemptId } = req.params;
    try {
        const pool = await poolPromise;
