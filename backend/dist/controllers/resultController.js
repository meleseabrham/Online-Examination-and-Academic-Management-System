...
export const getTeacherResults = async (req, res) => {
    ...
    try {
        ...
        if (semesterId)
            filterClause += " AND (e.SemesterId = @semesterId OR e.SemesterId IS NULL)";
        if (academicYearId)
            filterClause += " AND (ISNULL(e.AcademicYearId, sem.AcademicYearId) = @ayId)";
        if (gradeId)
            filterClause += " AND ISNULL(g.Id, 0) = @gradeId";
        if (sectionId) {
        ...
        if (!isAdmin)
            request.input('teacherId', sql.Int, Number(userId));
        if (semesterId)
            request.input('semesterId', sql.Int, Number(semesterId));
        if (academicYearId)
        ...
        if (examType && examType !== 'All')
            request.input('examType', sql.NVarChar, examType);
        const debugInfo = {
            isAdmin,
            userId,
            semesterId,
        ...
        const enrichedSubmissions = submissions.map(s => {
            ...
            const findSetting = (type, id) => id ? permissions.find(p => p.EntityType === type && String(p.EntityId) === String(id)) : null;
            ...
            if (aySetting)
                allowed = aySetting.SettingValue === 'true';
            if (semSetting)
                allowed = semSetting.SettingValue === 'true';
            if (gradeSetting)
                allowed = gradeSetting.SettingValue === 'true';
            if (sectionSetting)
                allowed = sectionSetting.SettingValue === 'true';
...
export const getStudentProgress = async (req, res) => {
    ...
    try {
        ...
        if (!isAdmin)
            request.input('teacherId', sql.Int, userId);
        if (semesterId)
                ...
                if (q.Type === 'MCQ' || q.Type === 'TF') {
                    ...
                    if (q.MatchingAnswer) {
                        ...
                        for (const p of pairs) {
                            ...
                            if (sAns === String(p.RightText || '').trim().toLowerCase() && sAns !== '')
                                qCorr++;
                        }
                    ...
                    if (Number(q.MarksAwarded) >= qPoints * 0.5)
                        correctCount++;
                }
            }
