...
export const lockExamAttempt = async (req, res) => {
    const { attemptId } = req.body;
    try {
        const pool = await poolPromise;
        await pool.request()
...
export const getStudentResults = async (req, res) => {
    ...
    try {
        ...
        `;
        if (statsAYId)
            statsQuery += " AND AcademicYearId = @ayId";
        if (statsSemId)
            statsQuery += " AND SemesterId = @semId";
        ...
        if (rankRes.recordset.length > 0) {
            rankingInfo = {
                ...
                average: Number(rankRes.recordset[0].Average || 0).toFixed(1)
            };
        }
